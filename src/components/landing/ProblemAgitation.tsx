    ...
    <section className="py-20 tech-border-top">
      <div className="section-container">
        <div className="text-center mb-6">
          <p className="text-brand-cta font-sans text-sm uppercase tracking-widest mb-3">Responda com sinceridade</p>
        ...
        {/* Dores Emocionais */}
        <div className="mb-8">
          <p className="text-sm font-sans text-brand-cta/70 uppercase tracking-widest mb-4 text-center">😔 Dores Emocionais</p>
          <div ref={ref} className="grid sm:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {emotionalPains.map((p, i) => (
              <div
                key={p.title}
                className={`glass-card p-6 border-brand-cta/20 hover:border-brand-cta/40 hover:shadow-[0_0_30px_rgba(255,100,50,0.3)] transition-all reveal ${isVisible ? "visible" : ""} stagger-${i + 1}`}
              >
                ...
                <h3 className="font-sans text-xl font-bold mb-2">{p.title}</h3>
        ...
        {/* Dores Práticas */}
        <div>
          <p className="text-sm font-sans text-brand-cta/70 uppercase tracking-widest mb-4 text-center">⚙️ Dores Práticas</p>
          <div className="grid sm:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {practicalPains.map((p, i) => (
              <div
                key={p.title}
                className={`glass-card p-6 border-brand-cta/20 hover:border-brand-cta/40 hover:shadow-[0_0_30px_rgba(255,100,50,0.3)] transition-all reveal ${isVisible ? "visible" : ""} stagger-${i + 3}`}
              >
                <div className="w-12 h-12 rounded bg-brand-cta/10 flex items-center justify-center mb-4">
                  <p.icon className="w-6 h-6 text-brand-cta" />
                </div>
        ...
        <div className="text-center mt-12">
          ...
          <p className="text-2xl font-sans font-bold mt-2">
